...
class WP_Widget_Text_Sidebar extends WP_Widget {

	function __construct() {
		$widget_ops = array('classname' => 'widget_text_sidebar', 'description' => __('Arbitrary text or HTML.'));
		$control_ops = array('width' => 400, 'height' => 350);
		parent::__construct('text', __('Text Sidebar'), $widget_ops, $control_ops);
	}
	...
	function widget( $args, $instance ) {
		...
		echo $before_widget;
		if ( !empty( $title ) ) { echo '<div class="heading_box">' . $title . '</div>'; } ?>
			<div class="block-content <?php if(empty($title)): ?>no-title<?php endif; ?>"><?php echo !empty( $instance['filter'] ) ? wpautop( $text ) : $text; ?></div>
		<?php
		echo '</div>';
	}
